...
**************************/
    ...
    var strHtm64_Update = "<br><font color='#FF00FF'>打印控件需要升级!点击这里<a href='/Scripts/Lodop/install_lodop64.exe' target='_self'>执行升级</a>,升级后请重新进入。</font>";
        ...
        var LODOP;
        var obj = document.getElementById("errorPrintMsg");
	try{	
	     //=====判断浏览器类型:===============
	     ...
	     if ((LODOP == null) || (typeof (LODOP.VERSION) == "undefined")) {
	             if (navigator.userAgent.indexOf('Chrome')>=0)
	                 obj.innerHTML = strHtmChrome + obj.innerHTML;
	             if (navigator.userAgent.indexOf('Firefox')>=0)
	                 obj.innerHTML = strHtmFireFox + obj.innerHTML;
	             if (is64IE) document.write(strHtm64_Install); else
	             if (isIE)   document.write(strHtmInstall);    else
	                 obj.innerHTML = strHtmInstall + obj.innerHTML;
	             return LODOP;
	     } else 
	         if (LODOP.VERSION < "6.1.8.3") {
	             if (is64IE) document.write(strHtm64_Update); else
	             if (isIE) document.write(strHtmUpdate); else
	                 obj.innerHTML = strHtmUpdate + obj.innerHTML;
	    	     return LODOP;
	     };
	     ...
	     if (is64IE)	
	         obj.innerHTML = "Error:" + strHtm64_Install + obj.innerHTML; else
	         obj.innerHTML = "Error:" + strHtmInstall + obj.innerHTML;
	     return LODOP; 
	};
}
