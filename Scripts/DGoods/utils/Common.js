define(["jquery", "../utils/SysUrl"], function ($, SysUrl) {
    ...
    //写诸葛日志
    function WriteAnalyze(actname, options) {
        if (window.zhuge != undefined) {
            var username = "匿名用户";
            if (userInfo && typeof (userInfo) === "object") {
                username = userInfo.UserName;
            }
            var opt = $.extend({ "name": username }, options);
            window.zhuge.track(actname, options);
        }
    }
    ...
    function checkCert(event, userInfo) {
        if (userInfo && typeof (userInfo) === "object") {
            if (userInfo.SystemCert.indexOf('A') < 0) {
                ...
                switch (userInfo.CertState) {
                    case 0:
                        msg = {
                            title: "请完成企业实名认证后再操作！",
                            desc: "进行商城交易需经过<span class=\"lightBlue\">企业实名认证</span>，您当前尚未进行此认证。",
                            button: {
                                cleanBtn: {
                                    visable: true,
                                    text: '取消',
                                ...
                                enterBtn: {
                                    ...
                                    url: '',
                                    onConfrim: function () {
                                        window.open(SysUrl.getUrl('/Users/ValidateInfo/BaseInfo'), "_self");
                                    }
                                }
                            }
                        };
                    ...
                    case 3:
                        msg = {
                            title: "认证未通过，无法操作！",
                            desc: "您的帐号未通过认证审核，原因：" + userInfo.CertNote + "。重新认证通过后才可以操作~",
                            button: {
                                cleanBtn: {
                                    visable: true,
                                    text: '取消',
